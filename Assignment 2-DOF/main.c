...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Anuj Sanathanan");
MODULE_DESCRIPTION("BLOCK DRIVER ASSIGNMENT 2");
...
static void dof_transfer(struct sbull_dev *dev, unsigned long sector,
...
static blk_status_t dof_request(struct blk_mq_hw_ctx *hctx, const struct blk_mq_queue_data* bd)   /* For blk-mq */
{
	...
	rq_for_each_segment(bvec, req, iter)
	{
		...
		dof_transfer(dev, pos_sector, num_sector,
 ...
 */
static int dof_xfer_bio(struct sbull_dev *dev, struct bio *bio)
{
	struct bio_vec bvec;
	...
	bio_for_each_segment(bvec, bio, iter) {
		...
		dof_transfer(dev, sector, (bio_cur_bytes(bio) / KERNEL_SECTOR_SIZE),
 ...
 */
static int dof_xfer_request(struct sbull_dev *dev, struct request *req)
{
	struct bio *bio;
	int nsect = 0;
    
	__rq_for_each_bio(bio, req) {
		dof_xfer_bio(dev, bio);
...
static blk_status_t dof_full_request(struct blk_mq_hw_ctx * hctx, const struct blk_mq_queue_data * bd)
{
	struct request *req = bd->rq;
	int sectors_xferred;
	...
	//while ((req = blk_fetch_request(q)) != NULL) {
		...
		sectors_xferred = dof_xfer_request(dev, req);
		ret = BLK_STS_OK; 
	done:
...
{
	int ret=0;
	printk(KERN_INFO "DOF drive: open \n");
	goto out;
...
static void dof_release(struct gendisk *disk, fmode_t mode)
{
	
	printk(KERN_INFO "DOF drive : closed \n");
...
static void sbull_exit(void)
{
	...
	unregister_blkdev(sbull_major, "dof");
	put_disk(device.gd);	
